...
const notifNum = document.querySelector(".notifNumber");
...
window.addEventListener("load", (event) => {
  notifContainer.addEventListener("click", () => updateNotifs());
});
...
async function updateNotifs() {
  notifs.forEach((n) => {
    console.log(n);
    //     if (!n.view) {
    //       fetch("http://localhost:19000/notif/" + n.id, {
    //         method: "PUT",
    //         headers: {
...
socket.on("newMessage", (data) => {
  if (data.receiverUserId === userGlobal.infos._id) {
    console.log(data, "dataNewMsg");
    const token = window.localStorage.getItem("token-auth");
    getMessagesByUser(userGlobal.infos, token).then((response) => {
      console.log(response);
      createMsgList(response.messages);
    });
    addNotif(data.notif, data.notifId);
  }
});

socket.on("updateDeleteMessage", (data) => {
  if (
    data.receiverUserId === userGlobal.infos._id ||
    data.senderUserId === userGlobal.infos._id
  ) {
    const token = window.localStorage.getItem("token-auth");
    getMessagesByUser(userGlobal.infos, token).then((response) => {
      console.log(response);
...
function checkToken() {
  ...
  fetch("http://localhost:19000/checkToken", {
    ...
    .then((res) => {
      if (res.msg === "token ok") {
        goToLogged(res.user);
        getMessagesByUser(res.user, token).then((response) => {
          createMsgList(response.messages);
        });
      } else {
        gotToLoggout();
...
function onClickSendMessage(e) {
  ...
  if (token) {
    sendMessage(userGlobal.infos, createMessage.value, token).then((res) => {
      console.log(res);
      getMessagesByUser(userGlobal.infos, token).then((response) => {
        console.log(response);
        createMsgList(response.messages);
...
function onClickEditMessage(e) {
  ...
  console.log("userGlobal", userGlobal);
  if (token) {
    editMessage(selectedMessage._id, createMessage.value, token).then((res) => {
      console.log(res);
      getMessagesByUser(userGlobal.infos, token).then((response) => {
        console.log(response);
        createMsgList(response.messages);
...
function onClickDelete(id) {
  ...
  console.log("userGlobal", userGlobal);
  if (token) {
    deleteMessage(id, token).then((res) => {
      console.log(res);
      getMessagesByUser(userGlobal.infos, token).then((response) => {
        console.log(response);
        createMsgList(response.messages);
...
function addNotif(msg, id) {
  ...
  notifNum.innerHTML = counter;
  notifContent.innerHTML = msg;
  notifContainer.style.display = "block";
}

function viewNotifs(e) {}
